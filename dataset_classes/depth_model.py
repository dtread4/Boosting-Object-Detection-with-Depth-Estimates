...
class DepthModel:
    ...
    @classmethod
    def initialize_depth_model(cls, model_name, precomputed_depth, **kwargs):
        """
        Class method to load the specified model

        Args:
            model_name: The name of the depth model to use
            precomputed_depth: Whether precomputed depth masks are being used
        ...
        if precomputed_depth:
            print(f"Using PRECOMPUTED depth maps for the [{model_name}] depth model\n")
            kwargs['model_name'] = model_name
            return cls._available_depth_models[PRECOMPUTED_NAME](**kwargs)
        ...
        print(f"Using depth (RGB-D inputs) generated at runtime by depth model: [{model_name}]\n")
...
@register_model(PRECOMPUTED_NAME)
class PrecomputedDepth(_DepthModelBase):
    ...
    def _load_model(self, verbose=False):
        """
        Does nothing but satisfies ABC requirements
    ...
    def calculate_depth_map(self, image_paths, verbose=False):
        ...
        # Load the files
        depth_masks = [
            self._load_precomputed_numpy_depth_mask(
                pth, 
                get_depth_outputs_path(os.path.dirname(image_paths[i]), self.model_name)) 
    ...
    def _load_precomputed_numpy_depth_mask(self, img_path, depth_dir):
        ...
        file_name_numpy = os.path.splitext(file_name_img)[0] + '.npy'
